        ...
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              addresses.all.length > 0 ? addresses.all.map((userAddress,index)=>
              
              <Box key={userAddress._id} className={
                addresses.selected === userAddress._id
                  ? 'address-item selected'
                  : ' address-item not-selected'
              } onClick={() => {
                setAddresses({
                  ...addresses,
                  selected: userAddress._id,
                  value: addresses.selected === userAddress._id,
                })
              }}>
                
                  <div id={userAddress._id} >
                    <Typography>{userAddress.address}</Typography>
                  </div>
                  <Button startIcon={<Delete />} size="large" variant="text" color="primary"  onClick={async () => {     await deleteAddress(token, userAddress._id)   }}  > DELETE</Button>
